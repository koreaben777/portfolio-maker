...
0.1.0 synthesis 단계는 원본 source가 여전히 존재하고 현재 hash가 같은 최신 승인 로컬 snapshot에서 master profile을 만듭니다. 누락, 변경, policy 차단, snapshot 유실 source는 재-ingestion 전까지 제외합니다. 남은 source별 `project_evidence` claim 하나를 만듭니다. GitHub activity는 후속 회사별 맞춤 생성 단계까지 artifact input이 아닙니다.
