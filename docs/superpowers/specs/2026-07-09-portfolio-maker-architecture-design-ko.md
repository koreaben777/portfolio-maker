...
날짜: 2026-07-09
상태: 승인된 역사적 아키텍처, pre-Phase-1 GitHub 문구는 현재 Phase 1 정책 계약으로 대체됨

이 문서는 승인된 아키텍처 설계와 pre-Phase-1 0.1.0 기준선을 기록합니다. GitHub discovery-only 문구는 역사적 기준선을 설명하며 현재 runtime 계약이 아닙니다. [현재 Phase 1 정책 및 근거 계약](2026-07-11-portfolio-maker-roadmap-phase-1-policy-evidence-github.md)이 권위 문서입니다. GitHub는 기본적으로 discovery metadata이며, policy 재검증을 통과한 confirmed public repository의 activity 중 정확히 명시 승인된 URL만 profile 또는 portfolio draft의 public-safe evidence로 사용할 수 있습니다. 이는 자동 project narrative를 만들지 않습니다.
...
- pre-Phase-1 기준선에서 GitHub 범위에는 repository, commit, pull request, issue, review, Actions 활동이 discovery metadata로 포함되며 snapshot, profile, portfolio draft에 들어가지 않습니다.
...
3. 사용자가 열람 금지 폴더와 repository를 제외할 수 있으며, source class 제외는 회사별 맞춤 생성 단계까지 후순위로 둡니다.
4. source approval이 명시되기 전까지 본문 ingestion을 차단합니다.
5. pre-Phase-1 기준선에서는 승인된 로컬 파일을 SQLite 및 최소 snapshot으로 ingest하고, GitHub repository와 activity는 discovery metadata로만 유지합니다.
...
- public/private repository 가시성을 표시합니다.
- organization resource 표시와 사용자의 직접 활동 우선순위는 후속 discovery 개선까지 후순위로 둡니다.
- pre-Phase-1 기준선에서는 repository와 activity를 discovery metadata로만 저장하며, GitHub 본문을 ingest하거나 artifact에 GitHub activity를 사용하지 않습니다.
...
- `version`: 선택적 approval format 버전이며, 없으면 `1`로 해석하고 `1`만 지원합니다.
...
pre-Phase-1 기준선에서는 GitHub approval setting이 discovery 가시성만 제어합니다. GitHub artifact input을 승인하는 기능은 아닙니다.

### 3. Ingestion

Ingestion은 승인된 로컬 파일 source만 읽습니다. 읽기 직전에 symbolic link, 일반 파일이 아닌 대상, canonical이 아닌 file URI, policy로 차단된 경로, MVP 크기 제한 초과 파일을 거부합니다.
...
pre-Phase-1 synthesis 단계는 원본 source가 여전히 존재하고 현재 hash가 같은 최신 승인 로컬 snapshot에서 master profile을 만듭니다. 누락, 변경, policy 차단, snapshot 유실 source는 재-ingestion 전까지 제외합니다. 남은 source별 `project_evidence` claim 하나를 만듭니다. GitHub activity는 후속 회사별 맞춤 생성 단계까지 artifact input이 아닙니다.
...
0.1.0 skeleton에는 다음 항목만 포함합니다.

- 승인된 source title
- 일반적인 포함 안내문
- placeholder role, technical approach, outcome
- 검토용 internal evidence reference

Project summary, problem/context, implementation details, 근거 기반 outcome, public-safe technology stack은 evidence-rendered portfolio generation이 구현될 때까지 후순위로 둡니다.
...
- public/private repository를 구분하며, organization 분류는 후순위로 둡니다.
- private repository는 명시적으로 허용된 경우에만 표시합니다.
- GitHub rate-limit 및 repository별 실패는 관련 없는 discovery 결과를 버리지 않고 report에 표시합니다.
- pre-Phase-1 기준선에서는 GitHub repository와 activity를 profile 또는 portfolio input으로 사용하지 않습니다.
...
`build-profile`은 기존 snapshot을 사용하기 전에 현재 approval, forbidden-path policy, 원본 file hash, 최신 snapshot 무결성을 다시 확인합니다.
