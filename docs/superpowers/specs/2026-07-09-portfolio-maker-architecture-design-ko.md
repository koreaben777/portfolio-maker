...
날짜: 2026-07-09
상태: 사용자 리뷰 대기 중인 작성 완료 스펙

이 문서는 승인된 아키텍처 설계를 기록합니다. 구현 계획은 사용자가 이 작성된 스펙을 검토하고 진행을 승인한 뒤에만 시작합니다.
...
5. 승인된 로컬 파일과 GitHub 활동을 SQLite 및 최소 snapshot으로 ingest합니다.
...
Domain layer는 커리어 근거와 생성된 주장을 표현합니다.

핵심 domain concept:

- `Source`: 로컬 파일, 폴더, repository, GitHub activity stream 또는 기타 발견 가능한 입력
- `SourceSnapshot`: 특정 시점 source에서 추출된 텍스트와 메타데이터
- `EvidenceItem`: 파일 범위, commit, PR, issue, review, README section 등 구체적인 근거 항목
- `GitHubActivity`: 정규화된 GitHub event 또는 object
- `Project`: 추론되었거나 사용자가 승인한 project grouping
- `CareerClaim`: 사용자의 작업, 기술, 역할, 성과에 대한 진술
- `ClaimEvidence`: claim과 supporting evidence의 관계
- `Artifact`: 생성된 output과 해당 source profile version
...
Synthesis 단계는 근거 기반 master profile을 만듭니다.

생성 항목:

- project summary
- technical skill inventory
- role and contribution summary
- implementation evidence
- activity evidence
- confidence value를 가진 career claim

충분한 근거가 없는 claim은 생략하거나 low-confidence/inferred로 표시해야 합니다.

### 5. Portfolio Drafting

Portfolio draft는 public-safe master-profile content에서 생성합니다.
...
스키마는 나중에 vector search를 붙일 수 있도록 `embedding_status`, `semantic_index_ref` 같은 field를 추가할 여지를 남겨야 합니다. Vector database는 MVP에 포함하지 않습니다.

## 근거 규칙

1. 중요한 claim은 evidence로 뒷받침되어야 합니다.
2. Evidence는 가능한 한 안정적인 locator를 가리켜야 합니다. 예: file path + hash, GitHub URL, commit SHA, PR URL, issue URL, review URL, workflow run URL.
3. 약한 claim은 low confidence로 표시하거나 생략해야 합니다.
4. Public-safe claim은 internal evidence와 분리해야 합니다.
5. Internal evidence는 private resource를 참조할 수 있지만, public artifact는 private raw path나 sensitive content를 노출하면 안 됩니다.
6. Source가 사라지거나 hash가 바뀌면 기존 confidence를 조용히 재사용하지 말고 source를 stale로 표시해야 합니다.
...
- private repository는 명시 승인 후에만 ingest합니다.
- rate limit은 pause/resume 방식으로 처리합니다.
- GitHub Actions는 code 또는 PR activity로 뒷받침되지 않는 한 약한 evidence로 취급합니다.
...
- Permission error는 기록하고 skip합니다.
- Parser failure는 raw content가 아니라 error type만 저장합니다.
- Rate limit은 progress를 저장하고 resume 가능하게 합니다.
- Network error는 auth/permission error와 구분합니다.
- Evidence가 부족하면 claim 생성을 막거나 claim confidence를 낮춥니다.
...
- master profile claim에 evidence reference가 있어야 합니다.
