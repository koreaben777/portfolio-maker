...
- Final documentation-only alignment commits: `e21328f` and `374a2a5`.
  They do not change runtime code or the recorded verification surface.
...
| `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src python -c 'import os; from pathlib import Path; from portfolio_maker.infrastructure.static_site import validate_static_output; root=Path(os.environ["PORTFOLIO_SMOKE_WORKSPACE"]); validate_static_output(root / "web/portfolio/dist"); html=(root / ".portfolio-maker/artifacts/portfolio.html").read_text(); assert html.count("<style>")==1 and "<link" not in html and "fetch(" not in html'` | passed; one inlined stylesheet, no `<link>`, no `fetch(` |
